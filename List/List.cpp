...
template <class T>
list_node<T>* List<T>::partition(list_node<T>* left,list_node<T>* right)
{
    list_node<T>* slow=left;
    list_node<T>* fast=left->next;
    T val=left->data;
...
template <class T>
void List<T>::listSort(list_node<T>* left,list_node<T>* right)
{
    if (left!=right)
    {
        list_node<T>* tmp=partition(left,right);
        listSort(left,tmp);
        listSort(tmp->next,right);
    }
}
...
{
    if(i>=this->list_len&&i< -(this->list_len))
    {
        throw "out of lens";
    }
    else
    {
        int I=(i+list_len)%list_len;
        list_node<T>* t=this->begin();
        while(I--)
        {
            t=t->next;
        }
